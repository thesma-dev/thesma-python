...
@click.option("--to", "to_date", default=None, help="Filter trades on or before this date (YYYY-MM-DD).")
...
def insider_trades_list(
    ...
    result = client.insider_trades.list(
        cik, from_date=from_date, to_date=to_date, person=person, page=page, per_page=per_page
    )
    fmt = ctx.obj["format"]
    if fmt == "json":
